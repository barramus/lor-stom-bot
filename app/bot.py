...
async def post_init(application):
    ...
    await application.bot.set_my_description(
        "Этот бот помогает стоматологу быстро сформировать и отправить ЛОР-врачу полную информацию "
        "о пациенте: жалобы, краткий анамнез, планируемую работу, а также прикрепить КТ сканы. "
        "Нажмите /start, чтобы начать консультацию."
    )
